...
function HomePage() {
	...
	const playMusic = async () => {
		...
		if (musicStatus) {
			...
			gainNode.gain.linearRampToValueAtTime(1, currentTime + 1)
			setMusicStatus(true)
		}
	}
	return (
		...
		<div className="relative w-full min-h-[100vh] overflow-hidden [mask-image:linear-gradient(180deg,#d9d9d9_70.62%,rgba(115,115,115,0)_100%)] ">
			<div>
				...
				<Image
					className={`absolute top-1 left-1 ${musicStatus ? 'opacity-70' : 'opacity-45'} transition`}
					width={20}
					height={20}
				...
				/>
				<Image
					className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 opacity-90 active:scale-105 z-20 transition "
					width={500}
					height={100}
